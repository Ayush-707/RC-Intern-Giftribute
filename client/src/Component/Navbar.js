...
const Header = () => {
  ...
  const toggleMenu = (menu) => {
    setMenuStates((prevState) => ({
      ...Object.fromEntries(
        Object.entries(prevState).map(([key]) => [key, key === menu && !prevState[key]])
      ),
    }));
  };
            ...
            {menuStates.isGiftsMenuOpen && (
              <div className="absolute z-50 top-8 left-0 bg-gray-800 py-2 px-4 rounded-lg text-white text-lg">
                {giftsArray.map((item, index) => (
                  <a href={item.link} className="block hover:scale-105 transform duration-200 ease-in-out" key={index}>
                    {item.text}
                  </a>
                ))}
              </div>
